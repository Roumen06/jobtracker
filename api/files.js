...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const blob = await put(`job-${jobId}/${formData.fileName}`, buffer, {
        access: 'public',
      ...
      const { rows } = await sql`
        ...
        VALUES (${jobId}, ${formData.fileName}, ${blob.url}, ${formData.fileType}, ${formData.fileSize})
        RETURNING *
      `;
    ...
    try {
      const { rows } = await sql`
        SELECT file_url FROM job_files WHERE id = ${fileId} AND job_id = ${jobId}
      `;
